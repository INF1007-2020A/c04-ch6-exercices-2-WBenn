...
def list_to_dict(some_list: list) -> dict:
    ...
    for i in range(len(some_list)):
        my_dict[some_list[i]] = i
    #UTILISER ENUMERATE EST MEILLEUR
    return my_dict
...
def create_list() -> list:
    ...
    my_list = [number for number in range(10000) if number < 15 or number > 350]
    return [my_list]
...
def compute_mse(model_dict: dict) -> dict:
    ...
    for key, value in model_dict.items():
        erreur_quad = 0
        for elem in value:
            erreur_quad += (elem[0]-elem[1])**2

        mse = erreur_quad/len(value)
        model_dict[key] = mse
